<?php
 class Tax_reports_model extends CI_Model 
{
	function payments_summary($client = 'all', $from_date = '', $to_date = '')
	{
		...
		$this->db->join('ci_tax_invoices', 'ci_tax_invoices.invoice_id = ci_payments.invoice_id');
		$this->db->join('ci_clients', 'ci_clients.client_id = ci_tax_invoices.client_id');
		if($client != 'all')
		{
			$this->db->where('ci_tax_invoices.client_id', $client);
		}
	...
	function client_stats($client_id = 0){
		...
		$this->db->from('ci_tax_invoices');
		$this->db->where('client_id', $client_id);
		$this->db->order_by('ci_tax_invoices.invoice_date_created', 'DESC');
		$invoices = $this->db->get();
		$counter = 0;
		$total_invoiced = 0; 
		$total_received = 0; 
	...
	function invoices_report($client_id = 0)
	{
		...
		foreach($invoices->result_array() as $count=>$invoice)
		{
			...
			$invoices_report[$counter]['invoice_client_ssm']		=	$invoice['client_ssm'];
			$invoices_report[$counter]['invoice_client_address']	=	$invoice['client_address'];
			$invoices_report[$counter]['invoice_client_phone']		=	$invoice['client_phone'];
			$invoices_report[$counter]['invoice_client_fax']		=	$invoice['client_fax'];
			$invoices_report[$counter]['invoice_client_gst']		=	$invoice['client_gst'];
	...
	function invoices_full_report($client_id = 0, $from_date = '', $to_date = '', $status = '')
	{
		$invoices_report = array();
		$this->db->select('*');
		$this->db->from('ci_tax_invoices');
	...
	function get_invoice_total_amount($invoice_id = 0)
	{
		...
		$this->db->select('*');
		$this->db->from('ci_tax_invoice_items');
		$this->db->where('ci_tax_invoice_items.invoice_id', $invoice_id);
		...
		$invoice_discount = $this->db->select('invoice_discount')->where('invoice_id', $invoice_id)->get('ci_tax_invoices')->row();
		...
		if($item['item_taxrate_id'] != 0)
		{
			...
			if (!empty($tax)) {
				$tax_total = (($item_total - $invoice_discount->invoice_discount) * ($tax->tax_rate_percent/100));
			}								
		}

		
		$invoice_totals['amount'] 		= $item_total - $invoice_discount->invoice_discount + $tax_total;
