 ...
 class Tax_invoice_model extends CI_Model 
{
	...
	function recent_invoices()
	{
		...
		$this->db->join('ci_clients', 'ci_clients.client_id = ci_tax_invoices.client_id');
		$this->db->limit(5);
		$this->db->order_by('ci_tax_invoices.invoice_id', 'DESC');
		$invoices = $this->db->get()->result_array();
		foreach($invoices as $invoice_count=>$invoice)
		{
			$invoice_totals = $this->get_invoice_total_amount($invoice['invoice_id']);
	...
	function overdue_invoices()
	{
		...
		$this->db->from('ci_tax_invoices');
		$this->db->join('ci_clients', 'ci_clients.client_id = ci_tax_invoices.client_id');
		$this->db->where('ci_tax_invoices.invoice_due_date < ', $today);
		$this->db->where('ci_tax_invoices.invoice_status', 'unpaid');
		$this->db->order_by('ci_tax_invoices.invoice_id', 'DESC');
		$invoices = $this->db->get()->result_array();
		foreach($invoices as $invoice_count=>$invoice)
		{
	...
	function get_invoice_details($invoice_id = 0)
	{
		...
		$this->db->from('ci_tax_invoices');
		$this->db->join('ci_clients', 'ci_clients.client_id = ci_tax_invoices.client_id');
		$this->db->where('ci_tax_invoices.invoice_id', $invoice_id);
		return $this->db->get()->row();
	}
	function get_invoice_data($invoice_id = 0)
	{

// 		echo var_dump($invoice_id);
// 		die();
		$this->db->select('*');
		$this->db->from('ci_tax_invoices');
		$this->db->join('ci_clients', 'ci_clients.client_id = ci_tax_invoices.client_id');
	...
	function get_invoice_items($invoice_id = 0)
	{
		...
		foreach($items as $item_count=>$item)
		{
			...
			if($item['item_taxrate_id'] != 0)
			{
				...
				$items[$item_count]['item_tax'] = $items[$item_count]['item_total'] * $tax_rate  ;
			}
			else
			{
	...
	function delete_invoice($invoice_id = 0)
	{
		...
		$this->db->delete('ci_tax_invoice_items');
		//delete payments
		$this->db->where('invoice_id', $invoice_id);
	...
	function previewinvoice($invoice_id = 0)
	{
		...
		$this->db->where('ci_tax_invoice_items.invoice_id', $invoice_id);
		$this->db->from('ci_tax_invoice_items');
		$this->db->join('ci_tax_rates', 'ci_tax_rates.tax_rate_id = ci_tax_invoice_items.item_taxrate_id', 'left');
	...
	function addpayment($invoice_id = 0, $payment_details = array())
	{
		...
		if($invoice_total['amount_due'] <= 0 )
		{
			...
			$this->db->where('invoice_id', $invoice_id)
					 ->update('ci_tax_invoices', $details);
		}
	}
	...
	function get_invoice_total_amount($invoice_id = 0)
	{
		...
		$invoice_totals['sub_total'] 			= $sub_total;
